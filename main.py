...
if __name__ == "__main__":
    ...
    if not configs.no_logging:
        logger = WandbLogger(
            ...
            id=configs.wandb_id,
            save_dir="/content/gdrive/Shareddrives/CV Project/The-Change-You-Want-to-See-Segmentation/work/rs/logs",
            name=configs.experiment_name
        )
        callbacks.append(ModelCheckpoint(dirpath=configs.model_save_path, 
                                          filename=configs.experiment_name + '-{epoch}-{val_loss:.2f}',
                                          monitor="val/overall_loss", mode="min", save_last=True,
                                          every_n_epochs=1, save_top_k=3,
                                          auto_insert_metric_name=True))
        callbacks.append(ModelCheckpoint(dirpath=configs.model_save_path,
                                            filename= configs.experiment_name + '-{epoch}-{global_step}',
                                            save_top_k=1,
                                            monitor="global_step",
                                            mode="max"))
