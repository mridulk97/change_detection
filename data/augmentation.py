"""
references - https://github.com/ragavsachdeva/The-Change-You-Want-to-See/blob/main/data/augmentation.py
"""
...
class AugmentationPipeline(nn.Module):
    def __init__(self, mode, path_to_dataset, image_transformation) -> None:
        ...
        # self.aff = K.augmentation.RandomRotation(
        #     degrees=30,
        #     align_corners=True,
        #     p=1.0,
        #     return_transform=True,
    ...
    def kornia_augmentation_function(self, input, type_of_image, image_index, transformation=None):
        if len(input.shape) == 3:
            input = torch.unsqueeze(input, dim=0)
        if self.mode in ["train", "val"]:
            return self.apply_train_augmentations(input, transformation)
        if self.mode == "test":
            return self.apply_test_augmentations(input, type_of_image, image_index)
        raise NotImplementedError(f"Unsupported mode {self.mode}")

    def apply_train_augmentations(self, input, transformation=None):
    ...
    def apply_test_augmentations(self, input, type_of_image, image_index):
        ...
        # print('precomputed_augmentation_path', precomputed_augmentation_path)
        if os.path.exists(precomputed_augmentation_path):
            augmentation_params = torch.load(precomputed_augmentation_path)
            # print('os.path.exists(precomputed_augmentation_path)', os.path.exists(precomputed_augmentation_path))
            # print('precomputed_augmentation_path', precomputed_augmentation_path)
        else:
            ...
            augmentation_params = {"aff": aff_params}
    ...
    def forward(self, image1_image_as_tensor, image2_image_as_tensor, image1_segmentations_transformed, image2_segmentations_transformed, image_file_name):
        # if self.image_transformation == "registered":
        #     return self.forward_registered(
        #         image1_image_as_tensor, image2_image_as_tensor, image1_segmentations_transformed, image2_segmentations_transformed
        #     )
        # if self.image_transformation == "identity":
        ...
        return self.forward_transformed(
            image1_image_as_tensor, image2_image_as_tensor, image1_segmentations_transformed, image2_segmentations_transformed, image_file_name
        )
    ...
    def forward_transformed(
        ...
        ) = self.kornia_augmentation_function(image1_image_as_tensor, "original", image_file_name)
        (image1_segmentations_transformed,_) = self.aff(image1_segmentations_transformed, self.aff._params)
        
        (
            image2_image_transformed,
            image2_image_transformation,
        ) = self.kornia_augmentation_function(image2_image_as_tensor, "inpainted", image_file_name)
        (image2_segmentations_transformed,_) = self.aff(image2_segmentations_transformed, self.aff._params)
        ...
        #     image1_segmentations_transformed,
        #     image1_image_transformation,
        # ) = self.kornia_augmentation_function(image1_segmentations_transformed, "original", image_file_name)
        # (
        #     image2_segmentations_transformed,
        #     image2_image_transformation,
        # ) = self.kornia_augmentation_function(image2_segmentations_transformed, "inpainted", image_file_name)
        
        
        # (image1_segmentations_transformed,_) = self.aff(image1_segmentations_transformed, image1_image_transformation)
        # (image2_segmentations_transformed,_) = self.aff(image2_segmentations_transformed, image2_image_transformation)
        # image1_segmentations_transformed = (
